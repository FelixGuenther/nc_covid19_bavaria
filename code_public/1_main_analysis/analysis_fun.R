...
#' Format results of STAN nowcast to obtain estimated ntInf and delay distribution
#' @param fit fitted STAN model
#' @param dataDays dates of the nowcast days for formatting
#' @return list containing tibbles of summaries of estimated ntInf and posterior median of delay distribution per day
...
#' Internal use, called by function estimate_nowcasts
#' @param dat data.frame with columns rep_date, rep_date_weekday and disease_start
#' @param now current date for performing the nowcast
#' @param begin_date starting date of nowcast
#' @param D maximum considered delay
#' @param predLag difference between `now` and the last desired prediction of number of disease onsets
#' @return list of data/preprocessed objects as required for different nowcasting models in STAN
prepare_data_list = function(dat,
  ...
  # Combin all data/preprocessed objects into list
  list(T = T+1,
       maxDelay = D,
...
#' estimates nowcasts based STAN model
#' @param imputed_data imputed data based on function perform_imputation
#' @param data_date the date when the data was received
#' @param safePredictLag difference between `now` and the last desired prediction of number of disease onsets, default to 2
...
#' @param dat_edit edited data frame with columns rep_week_local (week of reporting 
#' at local health authority), delay (reporting delay between disease onset and and reporting at 
#' local health authority)
#' @return summary of the delay distribution over the reporting weeks 
...
#' Estimate R(t)
#' Function to estimate the time-varying reproduction number R(t) based on draws of nowcast posterior.
#' @param nc_res Result of nowcasting (i.e., result of a function of of estimate_nowcasts)
#' @param data_date the date when the data was received
#' @return tibble with summary of estimated R(t) and 95% credibility interval
estimate_Rt = function(nc_res, data_date) {
  ...
  posterior_samples = pmax(cbind(do.call(rbind, 
                                         lapply(1:n_sample, 
                                                function(x) nc_res$nc_smry$ntInf$med[1:(length(nc_res$nc_smry$ntInf$med)-nc_res$add_inf$D+nc_res$add_inf$safePredictLag)])),
