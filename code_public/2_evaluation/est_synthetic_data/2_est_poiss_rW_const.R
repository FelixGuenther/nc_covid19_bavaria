...
# Load model
mod = readRDS("../stan_models/poisson_rWalk_const.rds")
...
res_list = future_lapply(sample(eval_dates), FUN = function(x) {
  ...
  tryCatch(estimate_nowcast(now=x, 
                            ...
                            predLag = 2, 
                            model = mod, 
                            mod_name = "poisson_rW_const"),
           error = function(e) e)
})
# save(res_list, file = "../../../results_public/2_evaluation/synthetic_data/poiss_rW_const.RData")
