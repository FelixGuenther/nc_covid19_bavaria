...

{
  ...
  if (checked == FALSE) {
    parameters <- R0::integrity.checks(epid, t, GT, begin, end, date.first.obs, time.step, AR=NULL, S0=NULL, methods="TD")
    begin <- parameters$begin
    end <- parameters$end
  }
	epid = R0::check.incid(epid, t, date.first.obs, time.step)
...
#  }
  ...
  for (s in 1:end.nb) {
	...
	return(structure(list(R=R,conf.int=conf.int, P=P, p=p, GT=GT, epid=epid.bak, import=import, pred=pred, begin=begin, begin.nb=begin.nb, end=end, end.nb=end.nb, data.name=DNAME, call=CALL, R.simu=R.simu, method="Time-Dependent", method.code="TD"),class="R0.R"))    #return everything
