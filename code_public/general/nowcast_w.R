...
nowcast <- function(now,when,data,dEventCol="dHospital",dReportCol="dReport",
  ...
  if ("bayes.trunc.ddcp" %in% method) {
     ...
     if (!is.null(control[["ddcp",exact=TRUE]][["Wextra",exact=TRUE]])) {
      ...
      if (is.null(control[["ddcp",exact=TRUE]][["mcmc",exact=TRUE]])) {
          control[["ddcp"]][["mcmc"]] <- c(burnin=2500,sample=10000,thin=1, adapt=1000)
      } else {
          if (!all(names(control[["ddcp",exact=TRUE]][["mcmc",exact=TRUE]]) %in% c("burnin","sample","thin", "adapt"))) {
              stop("mcmc options need names 'burnin', 'sample', 'thin' and 'adapt'")
          }
      }
  }
  ...
  if ("bayes.trunc.ddcp" %in% method) {
      ...
      for (t in 0:T) {
          ...
          # Week-end effects
          # W[t + 1, length(ddChangepoint) + 1, ] <- as.numeric( lubridate::wday(t02s[t+1] + 0:D) %in% c(1, 7))
...
###      browser()
      ...
      attr(delayCDF[["bayes.trunc.ddcp"]],"model") <- list(post.median=dt.surv.pm,W=W,lambda.post=lambda.post,tps=tps, mcmc_samples = samples, gamma=gamma, eta=eta)
